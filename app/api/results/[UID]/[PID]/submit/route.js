...
async function handlerPOST(req, context) {
  ...
  // -----------------------------------------------------------------------------------------------------------
  // Process HTMLString and determine number of hints used and encoded result
  ...
  const { HintsUsed, EncodedResult } = processHTMLString(HTMLString);
  ...
  // get the PID from the html string

  // read to see if the puzzle title and description exist in the puzzle table
  const getPuzzle = async (PID) => {
    const ItemParams = {
      TableName: "sbm-user-puzzle",
      Key: {
        PID: { S: PID },
      },
    };
    const data = await GetItem("sbm-reference-", ItemParams);
    return data.Item;
  };
